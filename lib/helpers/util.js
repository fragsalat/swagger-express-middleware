...
/**
 * Compary a value against a filter criteria which matches the value. If the filter criteria contains
 * a list where the value doesn't only one item from the list must match.
 * @param {*} value
 * @param {*} expected
 * @example matches([{test: 1, foo: [2, 3]}], {test: 1, foo: [3, 4, 5]})
 */
exports.matches = function matches(value, expected) {
  if (Array.isArray(expected)) {
    for (let i = expected.length; i--;) {
      // A logical or will be reflected by an array inside an array
      if (Array.isArray(expected[i])) {
        // Logical or means only one value has to match
        for (let j = expected[i].length; j--;) {
          if (matches(value, expected[i][j])) {
            return true;
          }
        }
      }
      // This value must match if it is a single one so it fails when it doesn't
      else if (!matches(value, expected[i])) {
        return false;
      }
    }
    return true;
    ...
    // Only one value has to match the expectation
    for (let i = value.length; i--;) {
      if (matches(value[i], expected)) {
        return true;
      }
    }
    return false;
    ...
    return Object.keys(expected).map(key => matches(value[key], expected[key])).indexOf(false) === -1;
  }
  else {
